...
function electronPrompt(options, parentWindow) {
    return new Promise((resolve, reject) => {
		...
		for (const type of ['textarea', 'counter', 'select', 'keybind', 'multiInput']) {
		//**** Textarea feature
            if (
                options_.type === type &&
		...
		//**** Textarea feature
		let window_is_modal = Boolean(parentWindow);
		if (options_.type == 'textarea') {
			window_is_modal = true;
			options_.resizable = false;
			options_.minWidth  = options_.width;
			options_.minHeight = options_.height;
			//options_.title = "Test textarea";
			options_.alwaysOnTop = true;
        }
	    //**** Textarea feature
