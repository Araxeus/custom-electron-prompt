...
function promptRegister() {
    ...
    switch (promptOptions.type) {
		...
		//*****	Textarea feature	
        case 'textarea':
            dataElement = promptCreateTextArea(promptOptions);
            break;
		//*****	Textarea feature	
        case 'input':
...
function promptSubmit() {
    ...
    switch (promptOptions.type) {
        case 'input':
		case 'textarea': //***** Textarea feature
        case 'select':
...
//**** Textarea feature
function promptCreateTextArea(options) {
    const dataElement = document.createElement('textarea');
    //dataElement.setAttribute('type', 'text');
    dataElement.value = options.value ?? '';
	
    //insert custom input attributes if in options	
    if (options.inputAttrs && typeof options.inputAttrs === 'object') {
		//options.inputAttrs["class"] = "textarea_field";
		//console.log("inputAttrs: " + JSON.stringify(options.inputAttrs));
		...
		dataElement.setAttribute("spellcheck", "false"); // checked
    }
...
function promptCreateMultiInput(parentElement) {
    ...
    for (const option of promptOptions.multiInputOptions) {
        ...
        if (option.inputAttrs) {
			// Counter in "multiInput"
			let element_type = "input";
			if (option.inputAttrs.type != undefined) {
			   element_type = option.inputAttrs.type;
			}
			//console.log(">> custom-electron-prompt element_type: " + element_type); 
		    switch(element_type) {
				// Counter in "multiInput"
				case "counter":
					el = promptCreateCounter(parentElement);
					break;
				// Counter in "multiInput"
				case "input":
